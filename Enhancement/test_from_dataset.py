...
parser = argparse.ArgumentParser(
    description='Image Enhancement using ReSCUE')
...
parser.add_argument(
    '--opt', type=str, default='Options/ReSCUE_LOL_V1.yml', help='Path to option YAML file.')
...
if dataset in ['SID', 'SMID', 'SDSD_indoor', 'SDSD_outdoor']:
    ...
    target_paths = natsorted(glob(os.path.join(
        target_dir, '*.png')) + glob(os.path.join(target_dir, '*.jpg')) +  glob(os.path.join(target_dir, '*.bmp')))
